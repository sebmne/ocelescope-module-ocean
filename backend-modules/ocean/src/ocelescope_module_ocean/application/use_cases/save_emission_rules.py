...
class SaveEmissionRulesCommand:
    ocel_id: str
    drafts: tuple[EmissionRuleDraft, ...]


class SaveEmissionRules:
    """Replaces the rules the user has defined for an OCEL."""
    ...
    def execute(self, command: SaveEmissionRulesCommand) -> tuple[EmissionRuleDraft, ...]:
        self._emission_rules_repository.save(command.ocel_id, command.drafts)
